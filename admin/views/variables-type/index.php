<?php

use yii\helpers\Html;
use yii\widgets\Pjax;
use yii\grid\GridView;
use yii\helpers\Url;
use common\helpers\Utils;

$this->title = 'Variables types list';
?>

<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <section class="feture_Tble-cst">
            <div class="row">
                <div class="col-sm-12">
                    <div class="Tble-cst--btn">
                        <a href="<?= Url::to(['/variables-type/create']);?>" class="add_new_btn" title="Add New Category">
                            <i class="fa fa-plus-circle" aria-hidden="true"></i> Add New
                        </a>
                    </div>
                    <div class="table-outer">
                        <?php Pjax::begin(); ?>
                        <?= GridView::widget([
                            'dataProvider' => $dataProvider,
                            'tableOptions' =>['class' => 'table variable-types-table'],
                            'layout' => "<div class='table-responsive'>{items}</div><div class='grid-view-pagination'>{pager}</div>",
                            'summary'=>'',
                            'pager' => [
                                'class' => '\yii\widgets\LinkPager',
                                'prevPageLabel'    => '<i class="fa fa-angle-left"></i>',
                                'nextPageLabel'    => '<i class="fa fa-angle-right"></i>',
                                'firstPageLabel'   => false,
                                'lastPageLabel'    => false,
                                'nextPageCssClass' => 'cst_arrow next',
                                'prevPageCssClass' => 'cst_arrow prev',
                                'firstPageCssClass'=> 'first',
                                'lastPageCssClass' => 'last',
                                'maxButtonCount'   => 5,
                                'options' =>  [
                                    'class' => 'pagination pagination_block',
                                ]
                            ],
                            'columns' => [
                                [
                                    'label' => 'Variable Name',
                                    'class' => 'yii\grid\DataColumn',
                                    'format' => 'raw',
                                    'value' => function ($data) {
                                        return ($data->vname!='')?$data->vname:'--';
                                    }
                                ],
                                [
                                    'label' => 'Variable Description',
                                    'class' => 'yii\grid\DataColumn',
                                    'contentOptions' => [
                                        'class' => 'gridviewDesc',
                                        'style' => 'width:100px;'
                                    ],
                                    'format' => 'raw',
                                    'value' => function ($data) {
                                        return ($data->vdesc!='')?$data->vdesc:'--';
                                    }
                                ],
                                [
                                    'label' => 'Default Selected',
                                    'class' => 'yii\grid\DataColumn',
                                    'format' => 'raw',
                                    'value' => function ($data) {
                                        if($data->vdefault == '1'){
                                            return '<span class="active__Status" title="Active">
                                                       <i class="fa fa-check" aria-hidden="true"></i>
                                                    </span>
                                                ';
                                        }else{
                                            return '<span class="unactive__Status" title="In Active">
                                                        <i class="fa fa-ban" aria-hidden="true"></i>
                                                    </span>';
                                        }

                                    }
                                ],
                                [
                                    'label' => 'Status',
                                    'class' => 'yii\grid\DataColumn',
                                    'format' => 'raw',
                                    'value' => function ($data) {
                                        if($data->vstatus == '1'){
                                            return '<span class="active__Status" title="Active">
                                                       <i class="fa fa-check" aria-hidden="true"></i>
                                                    </span>
                                                ';
                                        }else{
                                            return '<span class="unactive__Status" title="In Active">
                                                        <i class="fa fa-ban" aria-hidden="true"></i>
                                                    </span>';
                                        }

                                    }
                                ],
                                [
                                    'label' => 'Last Updated',
                                    'class' => 'yii\grid\DataColumn',
                                    'format' => 'raw',
                                    'value' => function ($data) {
                                        return ($data->updated_at!=NULL)?date('M d Y',strtotime($data->updated_at)):'';
                                    }
                                ] ,
                                [
                                    'class' => 'yii\grid\ActionColumn',
                                    'header' => 'Action',
                                    //{delete}
                                    'template' => '{edit}',
                                    'buttons'=>[
//                                        'delete'=>function ($url, $model, $key) {
//                                            return Html::tag('a', '<i class="fa fa-trash-o" aria-hidden="true"></i>',
//                                                [
//                                                    'class' => 'Tble-edit Tble-deleted',
//                                                    'data-pjax' => 0,
//                                                    'title' => 'Delete',
//                                                    'data-method' => 'post',
//                                                    'data-confirm' => 'Are you sure you want to archive this variables-type?',
//                                                    'href'=> Url::to(['variables-type/delete?id='.$model->id]),
//                                                ]
//                                            );
//                                        },
                                        'edit'=>function ($url, $model, $key) {
                                            return Html::tag('a', '<i class="fa fa-pencil-square-o" aria-hidden="true"></i>',
                                                [
                                                    'class' => 'Tble-edit',
                                                    'title' => 'Edit',
                                                    'href'=> Url::to(['variables-type/update?id='.$model->id]),
                                                ]
                                            );
                                        },
                                    ],
                                ]
                            ]
                        ]); ?>
                        <?php Pjax::end(); ?>
                    </div>
                </div>
        </section>
    </div>
</div>
