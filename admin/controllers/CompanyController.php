...
class CompanyController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            $uploaded_file = UploadedFile::getInstance($model, 'upload_image');
            if($uploaded_file){
                $file_name = Utils::getUniqueFileName().'.'.$uploaded_file->extension;
                $is_success = $uploaded_file->saveAs('../../uploads/company/'.$file_name);
                if($is_success){
                    $model->logo = 'uploads/company/'.$file_name;
                }
            }
